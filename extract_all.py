...
DIRECTION = {'південне', 'північне', 'східне', 'західне', 'центральне'}
DIR_CONTEXT = re.compile(r'напрям|околиц|курс|сторон|част[иьі]\b', re.U)
    ...
    for line in re.split(r'[\n;]+', text):
        ...
        for m in TOKEN.finditer(line):
            ...
            if any(a - 24 <= m.start() <= b for a, b in obl_spans):
                continue
            if low(tok) in DIRECTION and DIR_CONTEXT.search(low(line)):
                continue
