...
# Фізичне обмеження дальності. КАБ скидають з літака над своєю або окупованою
# територією, дальність планування — до ~90 км. Долетіти до Вінниччини чи
# Львівщини він не може: якщо геокодер поставив КАБ туди, це помилка розбору.
# Те саме для артилерії. Список — області, куди КАБи справді дістають.
KAB_OBLASTS = {
    'харківська', 'донецька', 'луганська', 'запорізька', 'херсонська',
    'сумська', 'чернігівська', 'дніпропетровська', 'миколаївська',
}
    ...
    for line in re.split(r'[\n;]+', text):
        ...
        for m in TOKEN.finditer(line):
            ...
            if r and r['name'] not in seen:
                # КАБ поза зоною досяжності — помилка, а не подія
                if w == 'kab':
                    ob = low(r.get('oblast_name', '')).replace(' область', '')
                    if ob not in KAB_OBLASTS:
                        continue
                seen.add(r['name'])
